...
#chat .mes_reasoning_details[data-state="thinking"] .mes_reasoning_header::before {
    ...
    box-shadow:
        inset 16px -6px 0px 0px rgba(255, 255, 255, 0.95),
        inset 14px -5px 6px 1px rgba(230, 240, 255, 0.6),
        inset 12px -4px 12px 2px rgba(200, 220, 255, 0.3);
...
`;
  ...
  function createCulm(svg, baseX, baseY, height, width, lean, color, nodeColor, age) {
    ...
    // Highlight sáng dọc thân (phản chiếu ánh trăng bạc)
    const hl = document.createElementNS(SVG_NS, "rect");
    hl.setAttribute("x", baseX - topW/3);
    hl.setAttribute("y", baseY - height);
    hl.setAttribute("width", (width * 0.25).toFixed(1));
    hl.setAttribute("height", height);
    hl.setAttribute("fill", "rgba(180, 210, 255, 0.08)");
    ...
    for (let i = 1; i <= nodeCount; i++) {
      ...
      if (i % 2 === 1 && i < nodeCount) {
        ...
        for (let j = 0; j < leafCount; j++) {
          ...
          const blueVal = age === 0 ? 60 + Math.floor(Math.random() * 20) :
                           age === 1 ? 50 + Math.floor(Math.random() * 15) :
                                        40 + Math.floor(Math.random() * 12);
          const greenVal = age === 0 ? 40 + Math.floor(Math.random() * 10) :
                           age === 1 ? 35 + Math.floor(Math.random() * 8) :
                                        30 + Math.floor(Math.random() * 5);
          leaf.setAttribute("fill", `rgb(${20 + age * 2}, ${greenVal}, ${blueVal})`);
          leaf.setAttribute("opacity", (0.8 + Math.random() * 0.2).toFixed(2));
          g.appendChild(leaf);
        }
      }
    }
    ...
    for (let k = 0; k < 3; k++) {
      ...
      leafTip.setAttribute("fill", age === 0 ? "#1a2533" : age === 1 ? "#141c26" : "#10151c");
  ...
  function createBambooGrove(container) {
    ...
    createCulm(svg, cx - 2, 197, R(148, 162), R(6, 8), R(-2, 3), "#18202b", "#10151c", 2);
    createCulm(svg, cx + 5, 197, R(130, 145), R(5, 7), R(0, 4), "#18202b", "#10151c", 2);

    // === LAYER 2: Vòng giữa (Trưởng thành, nghiêng 5-10°, bắt sáng nhẹ) ===
    createCulm(svg, cx - 15, 197, R(158, 175), R(7, 9), R(-9, -5), "#1e2a38", "#141c26", 1);
    createCulm(svg, cx - 22, 197, R(140, 155), R(6, 8), R(-6, -3), "#1e2a38", "#141c26", 1);
    createCulm(svg, cx + 12, 197, R(160, 178), R(8, 10), R(6, 12), "#1e2a38", "#141c26", 1);

    // === LAYER 3: Rìa ngoài (Non, nghiêng 15-35°, bắt sáng nhất dưới ánh trăng) ===
    createCulm(svg, cx - 32, 197, R(85, 110), R(4, 6), R(-18, -25), "#243548", "#1a2533", 0);
    createCulm(svg, cx - 42, 197, R(55, 75), R(3, 4.5), R(-28, -35), "#243548", "#1a2533", 0);
    createCulm(svg, cx + 22, 197, R(75, 95), R(3.5, 5), R(20, 30), "#243548", "#1a2533", 0);
    ...
    for (const mx of shootPositions) {
      if (Math.random() > 0.3) {
        ...
        shoot.setAttribute("fill", "#1a2533");
